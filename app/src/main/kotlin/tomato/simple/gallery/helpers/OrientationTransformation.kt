...
class OrientationTransformation(var orientation: Int) : BitmapTransformation() {
    override fun updateDiskCacheKey(messageDigest: MessageDigest) {
        messageDigest.update("OrientationTransformation:$orientation".toByteArray(Charsets.UTF_8))
    }
    ...
    companion object {
        fun getMatrix(orientation: Int): Matrix {
            val matrix = Matrix()
            when (orientation) {
                ORIENTATION_UNDEFINED, ORIENTATION_NORMAL -> {}
                ORIENTATION_ROTATE_270 -> matrix.postRotate(-90f)
                ...
                ORIENTATION_FLIP_HORIZONTAL -> matrix.postScale(-1f, 1f)
                ORIENTATION_FLIP_VERTICAL -> matrix.postScale(1f, -1f)
            }
            return matrix
        }

        fun whSwapped(orientation: Int) = orientation == ORIENTATION_ROTATE_90 ||
